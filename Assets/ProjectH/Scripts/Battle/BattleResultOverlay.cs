...
namespace ProjectH.Battle // 프로젝트 전투 영역
{
    ...
    public sealed class BattleResultOverlay : MonoBehaviour // 24일차 전투 결과 화면
    {
        ...
        private static Text CreateText(Transform parent, string name, string value, int size, FontStyle style, Color color) // 공통 결과 텍스트 생성 (RuntimeUiKit 위임)
        {
            return RuntimeUiKit.CreateText(parent, name, value, size, color, style).AlignByGeometry().BestFit(12); // 모양 정렬·자동 크기 텍스트 반환
        }

        private static Button CreateButton(Transform parent, string name, string label) // 공통 결과 버튼 생성 (RuntimeUiKit 위임)
        {
            Button button = RuntimeUiKit.CreateButton(parent, name, new Color(0.14f, 0.29f, 0.48f, 1f)); // 결과 버튼 생성
            Text text = CreateText(button.transform, "Label", label, 25, FontStyle.Bold, Color.white); // 버튼 라벨 생성
