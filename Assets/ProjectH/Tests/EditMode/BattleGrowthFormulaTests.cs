...
namespace ProjectH.Tests.EditMode // 프로젝트 EditMode 테스트 영역
{
    public sealed class BattleGrowthFormulaTests // 전투 성장 공식 테스트
    {
        ...
        [Test] // 성장 배율 테스트 지정
        public void GetLevelMultiplier_UsesLinearFivePercentGrowthAndCapsAtMaxLevel() // 레벨당 8퍼센트 성장 및 최대 레벨 상한 확인 (Day67 1차 밸런스에서 5% → 8%)
        {
            Assert.That(BattleGrowthFormula.GetLevelMultiplier(1), Is.EqualTo(1.00f).Within(0.0001f)); // 1레벨 배율 확인
            Assert.That(BattleGrowthFormula.GetLevelMultiplier(5), Is.EqualTo(1.32f).Within(0.0001f)); // 5레벨 배율 확인
            Assert.That(BattleGrowthFormula.GetLevelMultiplier(20), Is.EqualTo(2.52f).Within(0.0001f)); // 20레벨 배율 확인
            Assert.That(BattleGrowthFormula.GetLevelMultiplier(99), Is.EqualTo(2.52f).Within(0.0001f)); // 최대 초과 배율 상한 확인
        }
        ...
        public void ScaleStat_AppliesLevelGrowthAndRoundsToInteger() // 기본 스탯 성장 및 정수 반올림 확인
        {
            ...
            Assert.AreEqual(132, BattleGrowthFormula.ScaleStat(100, 5)); // 5레벨 성장값 확인 (Day67 1차 밸런스)
            Assert.AreEqual(252, BattleGrowthFormula.ScaleStat(100, 20)); // 20레벨 성장값 확인
            Assert.AreEqual(252, BattleGrowthFormula.ScaleStat(100, 99)); // 최대 초과 성장 상한 확인
        }
