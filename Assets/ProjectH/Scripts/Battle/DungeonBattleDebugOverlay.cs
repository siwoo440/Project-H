...
namespace ProjectH.Battle // 프로젝트 전투 영역
{
    ...
    public sealed class DungeonBattleDebugOverlay : MonoBehaviour // 27일차 던전 전투 테스트 정보 오버레이
    {
        ...
        private void BuildOverlay() // 던전 전투 테스트 정보 UI 생성
        {
            ...
            panelRect.sizeDelta = new Vector2(860f, 86f); // 테스트 정보 패널 크기 설정
            ...
            label.text = BuildLabel(); // 선택 던전 테스트 정보 표시
        }

        private static string BuildLabel() // 선택 던전 테스트 정보 문구 생성
        {
            string selectedId = DungeonSelectionRuntimeState.SelectedDungeonId; // 현재 선택 던전 ID 조회
            DungeonBattleTestProfile profile = DungeonBattleTestProfile.Get(selectedId); // 선택 던전 테스트 프로필 조회
            ...
            {
                dungeon = GameManager.Instance.Data.GetDungeon(profile.DungeonId); // 표시용 던전 데이터 조회
            }

            string sourceLabel = string.IsNullOrWhiteSpace(selectedId) ? "DIRECT→DG001" : profile.DungeonId; // 직접 전투 실행 표시 결정
            ...
            return $"{sourceLabel} · {dungeonName} | ENEMY HP x{profile.HealthMultiplier:0.00} / ATK x{profile.AttackMultiplier:0.00} / DEF x{profile.DefenseMultiplier:0.00} / RES x{profile.ResistanceMultiplier:0.00} | WIN GOLD {rewardGold} / EXP {rewardExp}"; // 테스트 정보 문구 반환
        }
    }
}
