...
namespace ProjectH.SaveSystem // 프로젝트 저장 영역
{
    ...
    public sealed class ChapterProgressSaveData // 메인 스토리 진행 (Day68 신규 — 어느 챕터의 몇 번째 단계인지)
    {
        ...
        [SerializeField] private string finishedChapterId = string.Empty; // 마지막으로 끝낸 챕터 (Day69 추가 — 업데이트로 새 챕터가 생기면 이어서 진행)
        ...
        public string FinishedChapterId => finishedChapterId ?? string.Empty; // 마지막으로 끝낸 챕터 반환
        ...
        public void Finish(string lastChapterId = "") // 준비된 챕터를 모두 본 상태 (마지막 챕터를 기록해 두면 새 챕터가 추가될 때 이어진다)
        {
            started = true; // 시작 기록
            chapterId = string.Empty; // 챕터 비움
            stepIndex = 0; // 단계 초기화
            finishedChapterId = lastChapterId ?? string.Empty; // 마지막 챕터 저장
        }

        public void EnsureDefaults() // 이전 저장 기본값 복원
        {
            if (chapterId == null) chapterId = string.Empty; // 챕터 복원
            if (finishedChapterId == null) finishedChapterId = string.Empty; // 마지막 챕터 복원
